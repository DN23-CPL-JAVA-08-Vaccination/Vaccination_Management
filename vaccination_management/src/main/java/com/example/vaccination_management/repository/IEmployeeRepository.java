...
public interface IEmployeeRepository extends JpaRepository<Employee, Integer> {
     ...
     */
    String SELECT_EMPLOYEE_LIST ="select e.id, e.name, e.address, e.birthday, e.gender, e.id_card as idCard, e.phone, p.name as position, a.email, a.username from employee e \n" +
            "join position p on p.id = e.position_id\n" +
            "join account a on a.id = e.account_id\n" +
            "where  e.enable_flag = 1 and e.name like ?";
     ...
     */
    String SELECT_EMPLOYEE_BY_ID ="select e.id, e.name, e.address, e.birthday, e.gender, e.id_card as idCard, e.phone, p.name as position, a.email, a.username from employee e    \n" +
            "             join position p on p.id = e.position_id   \n" +
            "             join account a on a.id = e.account_id   \n" +
            "             where  e.enable_flag = 1 and e.id = ?";
    @Query(value = SELECT_EMPLOYEE_BY_ID,countQuery = SELECT_EMPLOYEE_BY_ID,nativeQuery = true)
    EmployeeListDTO getInforById(int index);
