...
//        String regexName = "^[a-zA-Z0-9 ]*$";
        ...
        if (age < 15) {
            if (vaccinationHistoryDTO.getGuardianName() == null) {
                errors.rejectValue(fieldGuardianName, "guardianName.null", "Vui lòng nhập tên.");
            } else if (vaccinationHistoryDTO.getGuardianName().length() < 6) {
                errors.rejectValue(fieldGuardianName, "guardianName.length", "Tên phải lớn hơn 5 ký tự.");
            } else if (vaccinationHistoryDTO.getGuardianName().length() > 40) {
            ...
            if (vaccinationHistoryDTO.getGuardianPhone() == null) {
        ...
        if (vaccinationHistoryDTO.getPreStatus() == null) {
            errors.rejectValue(fieldPreStauts, "preStatus.null", "Vui lòng nhập trạng thái bệnh nhân.");
        } else if (vaccinationHistoryDTO.getPreStatus().length() > 100) {
