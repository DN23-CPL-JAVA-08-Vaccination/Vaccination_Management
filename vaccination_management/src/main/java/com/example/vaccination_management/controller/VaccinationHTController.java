...
//    private IVaccinationHistoryRepository iVaccinationHistoryRepository;
    ...
    public String finAll(@RequestParam(value = "page", defaultValue = "0") int page,
                         @RequestParam(value = "size", defaultValue = "1") int size,Model model){
...
//        sendEmailAdress(vaccinationHistory.getId(), model);
    ...
    public void sendEmailAdress(@PathVariable("id") int id,Model model){
     ...
     /**
     * VuongVV
     * send email when update status Disagree with vaccination of VaccinationHistory, admin after login
     */
    public void sendEmailAdressNot(@PathVariable("id") int id,Model model){
        VaccinationHistory vaccinationHistory=iVaccinationHTService.finById(id);
        String subject = "Thông báo yêu cầu tiêm chủng tiêm chủng không đồng ý " +vaccinationHistory.getVaccination().getVaccine().getName();
        String content = "Kính gửi Lỗi! <br>Thông báo trạng thái tiêm chủng "+vaccinationHistory.getVaccination().getVaccine().getName()+" của bạn  <span style='color: red'> "+vaccinationHistory.getVaccinationStatus().getName()+"</span>";
        iEmailService.sendEmail(vaccinationHistory.getPatient().getAccount().getEmail(), subject, content);
        model.addAttribute("message", "Email sent successfully to all accounts!");
    }
}
